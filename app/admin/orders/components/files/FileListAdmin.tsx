...
export default function FileListAdmin({ orderId }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await authedFetch(`/api/admin/orders/${orderId}/files`);
      ...
      toast.error(e?.message || "Не удалось загрузить файлы админа");
      setFiles([]);
    } finally {
      setLoading(false);
    }
  ...
  if (files.length === 0) return <div className="text-sm text-slate-500">Нет файлов</div>;
      ...
      {files.map((f) => (
        <div key={f.id} className="flex items-center justify-between gap-3 rounded-xl border bg-white p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="rounded-lg border px-3 py-1 text-sm hover:bg-slate-50" onClick={() => downloadDirect(getUrl(f), f.filename)}>
              Скачать
            </button>
